 ...
 * @param {IGatsbyImageData} imageData
 */
const getAspectRatio = imageData =>
...
const getSrc = (srcSet) => {
  if (srcSet) {
    ...
    const initialSource = allSources.filter(src => {
      return src?.[0].includes('100w') || src?.[0].includes('1x');
    });
 ...
 * @param imageData
 */
const getAllExtraSrcSets = (imageData) => {
  if (imageData.images?.sources && Array.isArray(imageData.images?.sources)) {
    return imageData.images?.sources.reduce((srcSets, sourceImage) => {
      ...
      if (sourceType && !(possibleExtraSrc in srcSets)) {
        const extraSrc = getSrc(sourceImage?.srcSet);
        // @ts-ignore
        srcSets[possibleExtraSrc] = extraSrc;
      }
      return srcSets;
    }, {});
  }
  ...
  const {placeholderImage, oldImage} = useStaticQuery(
    graphql`
      query {
        ...
        oldImage: file(relativePath: { eq: "seamless-bg-desktop.jpg" }) {
          childImageSharp {
